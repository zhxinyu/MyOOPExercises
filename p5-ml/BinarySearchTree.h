...
public:
  ...
  BinarySearchTree()
    : root(nullptr) { std::cout<<"Default Constructor here."<<std::endl;}

  // Copy constructor
  BinarySearchTree(const BinarySearchTree &other)
    : root(copy_nodes_impl(other.root)) { std::cout<<"Copy Constructor here."<<std::endl;}
  ...
  public:
    Iterator()
      : root(nullptr), current_node(nullptr) {std::cout<<"Default iterator constructor."<<std::endl;}
  ...
  private:
    ...
    Iterator(Node *root_in, Node* current_node_in, Compare less_in)
      : root(root_in), current_node(current_node_in), less(less_in) { std::cout<<"Customized Iterator Constructor"<<std::endl;}
...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
      ...
      if (empty_impl(node)){
      ...
      else if (empty_impl(node->left) & empty_impl(node->right)){
          Node * tmp = new Node;
          tmp->datum =node->datum;
          tmp->left = nullptr;
          tmp->right =nullptr;
          return tmp;
      }
          ...
          Node * tmp = new Node;
          tmp->datum = node->datum;
          tmp->left  = copy_nodes_impl(node->left);
          tmp->right  = copy_nodes_impl(node->right);
          return tmp;
      }
    //assert(false);
  }
...
//    assert(false);
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
      ...
      assert(check_sorting_invariant_impl(node,less));
...
//      Iterator iter = min_element_impl(node);
//      while (*iter<item){
//          ++iter;
//      }
//      iter.current_node->left = tmp;
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
      ...
      else if (empty_impl(node->left) & empty_impl(node->right)) {return true;}
      else if (empty_impl(node->left)){
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      ...
      if (less(max_element_impl(node)->datum,val) | 
        ((!less(val,max_element_impl(node)->datum))&(!less(max_element_impl(node)->datum,val))))
          {return nullptr;}

      //assert(!find_impl(node,val,less));
      else if (empty_impl(node->left)&empty_impl(node->right)){
          return node;
      }
      ...
      else if (less(val,max_element_impl(node->left)->datum)){return min_greater_than_impl(node->left,val,less);}
      else{ return node;}
}
