...
int main() {
  ...
  // std::pair <Map<string, int>::Iterator,bool> map1;
  // std::pair <Map<Duck, string, DuckWealthLess>::Iterator,bool> map2;
  it = map.insert({"", 1}).first;
  b = map.insert({"quack", 2}).second;

  //map1 = map.insert({"", 1});

  //map2 = map.insert({"quack", 2});
  //cout<<(*(map1.first)).first<<(*(map1.first)).second<<map1.second<<endl;
...
cout<<b<<endl;
  ...
  cout << b << st <<const_duck_map.size()<<endl;
